...
from razordl.core.engine.single_model.workgroup import WorkGroup
from razordl.ops.distributed.utils import all_gather_object
from razordl.ops.loss.distributed import DistCrossEntropyLoss


class LMWorkGroup(WorkGroup):
    """Single-model next-token LM training skeleton."""

    model_group_class = None
    criterion_class = DistCrossEntropyLoss

    def __init__(self, config):
        super().__init__(config)
        if self.model_group_class is None:
            raise TypeError(f"{type(self).__name__}.model_group_class must be set")
        ...
        self.criterion = self.criterion_class(ignore_index=-100)
